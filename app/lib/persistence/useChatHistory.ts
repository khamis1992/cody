            ...
            if (startingIdx > 0) {
              ...
              filteredMessages = [
                ...
                },
                {
                  ...
                  content: `Code Launch restored your chat from a snapshot. You can revert this message to load the full chat history.
  ...
  return {
    ...
    importChat: async (description: string, messages: Message[], metadata?: IChatMetadata) => {
      ...
      try {
        ...
        window.location.href = `/coding/${newId}`;
...
function navigateChat(nextId: string) {
   ...
   *
   * `navigate(`/coding/${nextId}`, { replace: true });`
   */
  const url = new URL(window.location.href);
  url.pathname = `/coding/${nextId}`;
