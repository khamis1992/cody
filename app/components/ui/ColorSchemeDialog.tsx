...
export interface ColorSchemeDialogProps {
  ...
  onSave?: (scheme: DesignScheme) => void;
}
...
export const ColorSchemeDialog: React.FC<ColorSchemeDialogProps> = ({
  ...
  const setIsDialogOpen = isControlled ? onOpenChange || setInternalIsOpen : setInternalIsOpen;
  ...
  const handleSave = () => {
    const newScheme = { palette, features, font };
  ...
  return (
    <div>
      <IconButton title="Design Palette" className="transition-all" onClick={() => setIsDialogOpen(true)}>
